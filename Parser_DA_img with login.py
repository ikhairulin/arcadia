...
def parse_deviant_art(key, qty):
    ...
    logging = session.post(url_login, data = datas, headers = header)

    cookies_dict = [{"domain": key.domain, "name": key.name, "path": key.path, "value": key.value} for key in session.cookies]
